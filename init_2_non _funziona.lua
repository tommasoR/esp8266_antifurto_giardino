...
srv:listen(80,function(conn) 
    ...
    conn:send("<h1> Allarme giardino<BR>Server is working!<BR>allarmOn="..allarmOn)
	--conn:send(allarmOn)
	if (allarmOn > 0) then
		s=tmr.time()-timeOn
		ore=s/3600
		...
		conn:send("<BR>Ultimo Allarme ore passate:"..ore" Minuti:"..minuti)
		--conn:send(ore)
		--conn:send(" Minuti:")
		--conn:send(minuti)
		if allarmOn==2 then
			conn:send("<BR>Ha suonato per Minuti:"..(timeOff-timeOn)/60)
